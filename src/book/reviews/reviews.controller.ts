...
export class ReviewsController {
  ...
  public async getReviewsForBook(
      ...
      const reviews = await this.reviewsService.getReviewsForBook(bookId, page, pageSize);
  ...
  @ApiResponse({ status: 200, description: 'Returns the average rating for a book.' })
  public async getAverageRatingForBook(
      @Param('bookId') bookId: number,
  ): Promise<StandardResponse<number | null>> {
      const averageRating = await this.reviewsService.getAverageRatingForBook(bookId);
